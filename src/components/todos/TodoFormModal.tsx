...
interface Props extends Partial<Pick<Todo, 'description' | 'completed'>> {
  title: string
  onSubmitTodo: (todo: Pick<Todo, 'completed' | 'description'>) => Promise<void>
  onClose: () => void
}
...
export const TodoFormModal = ({
  ...
  const handleSubmit = async (_: unknown, formData: FormData) => {
    ...
    await onSubmitTodo({ completed: !!completed, description })
  }
