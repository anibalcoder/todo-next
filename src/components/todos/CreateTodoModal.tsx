...
interface Props {
  createTodo: ({
    description,
    completed,
  }: Pick<Todo, 'description' | 'completed'>) => Promise<void>
  onCloseModal: () => void
}

export const CreateTodoModal = ({ createTodo, onCloseModal }: Props) => {
  const handleSubmitTodo = async (_: unknown, formData: FormData) => {
    const description = formData?.get('description') as string
    const completed = formData?.get('completed') as string

    await createTodo({ description, completed: !!completed })
  }
